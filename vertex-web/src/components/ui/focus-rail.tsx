...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[700px] w-full flex-col overflow-hidden bg-transparent text-white outline-none select-none overflow-x-hidden rounded-3xl border border-white/5",
        className
      )}
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[220px] md:w-[300px] rounded-2xl border-t border-white/20 bg-neutral-900 shadow-2xl transition-shadow duration-300",
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-neutral-900/80 p-1 ring-1 ring-white/10 backdrop-blur-md">
              <button onClick={handlePrev} className="rounded-full p-3 text-neutral-400 hover:bg-white/10 hover:text-white transition active:scale-95">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="min-w-[40px] text-center text-xs font-mono text-neutral-500">
                {activeIndex + 1} / {count}
              </span>
              <button onClick={handleNext} className="rounded-full p-3 text-neutral-400 hover:bg-white/10 hover:text-white transition active:scale-95">
                <ChevronRight className="h-5 w-5" />
              </button>
            </div>
            {activeItem.href && (
              <Link href={activeItem.href} className="group flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-black hover:scale-105 transition active:scale-95">
                Explore
